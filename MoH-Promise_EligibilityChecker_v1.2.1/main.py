...
PAYER_REGEX = re.compile(
    r"(?P<UPMC>UPMC)|"
    r"(?P<KEYSTONE>KEYSTONE)|"
    r"(?P<AMERIHEALTH>AMERIHEALTH|CARITAS)|"
    r"(?P<WELLNESS>WELLNESS|CENTENE)|"
    r"(?P<WAIVER>WAIVER|COMMUNITY HEALTHCHOICES|CHC)"
)
...
def validate_excel_columns(input_path):
    try:
        ...
        if "Detail Data" not in xls.sheet_names:
            return ["Tab 'Detail Data' missing from Excel"]
        df = pd.read_excel(xls, sheet_name="Detail Data", nrows=0)
...
def prepare_output_folder(
    input_excel_path: str, timestamp: str, output_base_folder: str
) -> Tuple[Path, Path]:
...
def normalize_payer(contract: str) -> set:
    upper_name = contract.upper().strip()
    match = PAYER_REGEX.search(upper_name)
    if not match:
        return {upper_name}
        
    matched_group = match.lastgroup
    if matched_group == "UPMC":
        return {"UPMC"}
    elif matched_group == "KEYSTONE":
        return {"KEYSTONE FIRST"}
    elif matched_group == "AMERIHEALTH":
        return {"AMERIHEALTH"}
...
def extract_results(page, log_callback=None):
    ...
    # 1. Wait for the main eligibility container to load (confirms search completed)
    page.wait_for_selector(
        "#dnn_ctr1732_Eligibility_Table6",
        state="visible",
        timeout=20000,
    )

    # 2. Check if the Managed Care table exists
    gv_summary = page.query_selector("#dnn_ctr1732_Eligibility_gvSummary")
    if not gv_summary:
        if log_callback:
            log_callback(
                "ℹ No active Managed Care results found for this member lookup."
            )
        return {
            "result_rows": [],
            "insurance_names": [],
    ...
    # 3. Extract rows from Managed Care table
    rows = gv_summary.query_selector_all("tbody tr:not(:first-child)")
    for row in rows:
...
# ----------------------------
def check_results(
    extracted_data, row_contract, start_date_str, end_date_str, page, log_callback=None
):
    discrepancy = "No"
    penalty = "No"
    ...
    contract_set = normalize_payer(row_contract)

    match_found = False
    last_checked_insurance = "No insurance found"

    for insurance_name in insurance_names:
        insurance_set = normalize_payer(insurance_name)
        last_checked_insurance = insurance_name
        if contract_set.intersection(insurance_set):
            match_found = True
            break
    ...
    if not waiver_found:
        ...
        if log_callback:
            log_callback(
                f"⚠️ Waiver Discrepancy: Portal insurance name does not contain 'WAIVER'"
            )
    ...
    if date_discrepancy and log_callback:
        log_callback(
            f"⚠️ Date Range Discrepancy: Portal metrics do not match runtime requirements ({start_date_str} - {end_date_str})"
        )
    ...
    if discrepancy == "Yes" or penalty == "Yes":
        if log_callback:
            log_callback(
                f"❌ Row Summary Flagged: Record saved with an explicit Discrepancy or Penalty notice."
            )
    else:
        if log_callback:
            log_callback(
                f"🟢 Perfect Match: No discrepancies or penalties found for this member."
            )
...
# ----------------------------
def take_screenshot(page, output_folder, filename_prefix, log_callback=None) -> bool:
    try:
        page.wait_for_selector("#dnn_ctr1732_Eligibility_Table6", timeout=15000)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if entry_limit is not None:
                try:
                    limit_val = int(entry_limit)
                    if limit_val > 0:
                        input_rows = input_rows[:limit_val]
                        if log_callback:
                            log_callback(f"ℹ️ Entry limit specified. Traversing first {limit_val} rows.")
                except ValueError:
                    pass
        except Exception as e:
            if log_callback:
        ...
        has_historical_session = (
            progress_file_raw.exists() and progress_file_raw.stat().st_size > 0
        )
        ...
        try:
            try:
                ...
                if log_callback:
                    log_callback(
                        f"⚠️ Navigation Note: Eligibility Search target skipped or already active: {e}"
                    )
            ...
            for idx, row in enumerate(input_rows, 1):
                ...
                if stop_check and stop_check():
                    if log_callback:
                        log_callback(
                            "🛑 Automation run interrupted by user action request."
                        )
                    break
                ...
                if log_callback:
                    log_callback(
                        f"🔍 Processing row {idx}/{len(input_rows)}: {fullname} ({member_id_raw}) {dob}"
                    )
                ...
                try:
                    ...
                    discrepancy, penalty = check_results(
                        extracted_data,
                        row_contract,
                        start_date_str,
                        end_date_str,
                    ...
                    output_row.update(
                        {
                            "Insurance Name": "Portal/Network Error",
                            "Begin Date": "Portal/Network Error",
                            "End Date": "Portal/Network Error",
                            "Discrepancy": "Portal/Network Error",
                            "Penalty": "Portal/Network Error",
                            "Screenshot Taken": "No",
                        }
                    )
                    progress_writer.writerow(output_row)
