...
def load_file_statistics(file_path):
    ...
    try:
        ...
        if "Detail Data" not in xls.sheet_names:
            ...
            log_to_gui("  ❌ Missing Tab: Tab 'Detail Data' was not found in the selected Excel file.", "error")
            return

        df = pd.read_excel(xls, sheet_name="Detail Data", dtype=str, keep_default_na=False).fillna("")
        total_rows = len(df)
        ...
        if missing:
            log_to_gui(f"  ❌ Missing required columns in 'Detail Data': {', '.join(missing)}", "error")
        else:
...
# ============================================================
def run_wrapper(excel_file, output_folder_path, entry_limit=None):
...
def start_automation():
    ...
    # Parse and validate entry size if provided
    entry_limit_raw = entry_size_var.get().strip()
    entry_limit = None
    if entry_limit_raw:
        try:
            entry_limit = int(entry_limit_raw)
            if entry_limit <= 0:
                log_to_gui("❌ Entry Size must be a positive integer.", "error")
                return
        except ValueError:
            log_to_gui("❌ Entry Size must be a valid integer number.", "error")
            return
    ...
    worker_thread = threading.Thread(
        target=run_wrapper,
        args=(excel_file, output_f, entry_limit),
        daemon=True
    )
    worker_thread.start()
...
_label(main_frame, "Entry Size (Optional)").grid(row=4, column=0, sticky="w", pady=8, padx=(0, 16))
entry_size_entry = _entry(main_frame, entry_size_var, width=18)
entry_size_entry.grid(row=4, column=1, sticky="w", pady=8, padx=(0, 10), ipady=6)

# Elegant Instructions Panel
hint_frame = tk.Frame(main_frame, bg=COLOR_HINT_BG)
hint_frame.grid(row=5, column=0, columnspan=3, sticky="ew", pady=(16, 0))
...
hint_label = tk.Label(
    hint_frame,
    text=(
        "Step 1: Select your source Excel file (must contain a 'Detail Data' sheet), output folder, and optionally set the 'Entry Size' limit.\n"
        "Step 2: Click 'Connect Browser' to launch Edge and load the Promise login page. Log in if needed.\n"
        "Step 3: Click 'Start' to begin lookups.\n"
        "Step 4: Click 'Stop' to halt the lookup process at any time. The system will immediately finalize the Excel sheet and save your current progress, allowing you to resume later.\n\n"
        "💡 Tip: You do not need to close or reconnect the browser to start a rerun; the system will automatically reuse the open tab.\n"
        "🔄 Resetting & Re-runs: Delete the '_progress.csv' file to start from the beginning. To re-run specific rows/patients, simply open the '_progress.csv' file and delete those rows.\n"
        "⚡ Large Files: Set the 'Entry Size' limit to 500 when processing files with more than 500 records to ensure stable runs."
    ),
...
button_container = tk.Frame(main_frame, bg=COLOR_BG)
button_container.grid(row=6, column=0, columnspan=3, sticky="ew", pady=20)

btn_frame = tk.Frame(button_container, bg=COLOR_BG)
btn_frame.pack(anchor="center")

button_kwargs = dict(
    font=FONT_BUTTON, relief="flat", bd=0,
    width=16, pady=9, cursor="hand2",
...
progress_frame.grid(row=7, column=0, columnspan=3, sticky="ew", pady=(0, 16))
...
_label(main_frame, "Activity Log", font=("Segoe UI", 10, "bold"), fg=COLOR_TEXT).grid(
    row=8, column=0, sticky="w", pady=(6, 8),
)
